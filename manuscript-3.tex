...
\[\left\lVert I\mu \right\rVert_{L^0_b(\mathcal A)^\ast} \ge \int\limits \sum\limits_{E\in\mathcal E} \operatorname{sign}(\mu(E)) \chi_E\mathrm d\mu = \sum\limits_{E\in\mathcal E} \left\lvert \mu(E) \right\rvert. \]
%By choosing \(f\coloneqq \chi_{A_+} - \chi_{A_-}\), where \(A_+\) and \(A_-\) are the sets out of the Hahn-Jordan decomposition of \(\mu\) we get
%\[\mu(f) = \mu(A_+) - \mu(A_-) = \left\lVert \mu \right\rVert\]
By taking the supremum over all those collections \(\mathcal E\) we get \(\left\lVert I\mu \right\rVert = \left\lVert \mu \right\rVert\). Further the bijectivity follows from the fact \(I\circ I^{-1} =\operatorname{id}_{L^0_b(\mathcal A)^\ast}, I^{-1}\circ I = \operatorname{id}_{ba(\mathcal A)}\).  % \(I\mu_F = F\) and \(\mu_\)
...
\[\iota^\ast\colon L^\infty(\mu)^\ast \to L^0_b(\mathcal A)^\ast \cong ba(\mathcal A)\]
...
\[\operatorname{ran}(\iota^\ast) = \operatorname{ker}(\iota)^\perp = \left\{ F\in L^0_b(\mathcal A)^\ast\mid F(f) = 0 \text{ for all } %f \text{ s.t. }
...
\[\iota^\ast\colon ba(\mathcal B) \cong L^0_b(\mathcal B)^\ast \to\!\!\!\!\!\to \mathcal C_b(X)^\ast, \quad \mu\mapsto \big( f\mapsto \mu(f)\big)\]
